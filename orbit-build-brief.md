...
### Milestone 18 — Proactive assistant (memory over time)
Milestone 16's Orbit Assistant is reactive (v1): it answers a question from a live snapshot of the current pipeline, nothing more. Xo wants an eventual version that "keeps track" - notices what changed since she last checked, flags drift over time, without being asked. This needs two things Orbit doesn't have yet: (1) periodic stored snapshots of pipeline state, not just live-computed-on-request data, so there's a "before" to compare against - overlaps with Milestone 17's need for accumulated history; (2) a scheduled job (same proven pattern as Milestone 13's gate-alerts cron) that runs the comparison and surfaces it, either inside the assistant panel ("since you last checked...") or pushed like an alert. Not a different platform or toolset - an extension of patterns already working in this app. Revisit once Milestone 16 is in real use and there's enough history accumulated (same precondition as Milestone 17) to make "tracking change over time" meaningful rather than noise.
